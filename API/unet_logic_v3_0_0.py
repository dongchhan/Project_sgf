...
class SAM_for_Droplet():
    ...
    def get_seg_droplets (self, poly = False) :
        ...
        for i, cnt_i in enumerate(contours_i) : # 일단 전체사이즈로 만들고 나중에 crop으로 고치기
            ...
            if edge_inter.sum() > 0 :
                ...
                if inter_area > 0 :
                    ...
                    # 품질관리용 개선, min_diameter_thr 미적용
                    if (shape_dist < shape_dist_thr) & (diameter < self.max_diameter_thr) : #& (diameter >= self.min_diameter_thr) 
                        ...
                        if (dic["Edge"] == False) & (dic["Convexity"] > 0.9) & (dic["Solidity"] > 0.9) & (dic["Circularity"] > 0.75) :
                            ...
                            # df_seg_raw
                            self.dic_seg_raw_list.append(dic)
                            self.cnts.append(inter_cnt)

                            # draw Images : Output 이기 때문에, min_diameter_thr 이상만 draw
                            if (diameter >= self.min_diameter_thr) :
        ...
        try :
            # df_seg_raw
            self.df_seg_raw = pd.DataFrame(self.dic_seg_raw_list)
            self.df_seg_raw['File'] = self.file
            self.df_seg_raw = self.df_seg_raw[["File", "Segment", "cnt_area", "Diameter (um)", "Major diameter (um)", "Minor diameter (um)", "Area (um2)", "Perimeter (um)", "Circularity", "Convexity", "Solidity", "Aspect ratio", "Center of moment"]]

            # df_seg
            self.df_seg = self.df_seg_raw[self.df_seg_raw['Diameter (um)'] >= self.min_diameter_thr ]

        except :
            self.df_seg_raw = pd.DataFrame(columns = [["File", "Segment", "cnt_area", "Diameter (um)", "Major diameter (um)", "Minor diameter (um)", "Area (um2)", "Perimeter (um)", "Circularity", "Convexity", "Solidity", "Aspect ratio", "Center of moment"]])
...
def unet_main(image_file_name, option_min_diameter_thr, manual_scale = None, manual_scalebar = None):
    ...
    res = {
        ...
        "output_raw_csv_detail" : analyzer.df_seg_raw, # 신규
